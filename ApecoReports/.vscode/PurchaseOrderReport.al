report 50115 "Purchase Order Schneider"
{
    DefaultLayout = RDLC;
    RDLCLayout = './PurchaseOrder.rdl';
    Caption = 'Purchase_Order';
    ...
    dataset
    {
        dataitem("Purchase Header"; "Purchase Header")
        {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    dataitem(RoundLoop; "Integer")
                    {
                        ...
                        begin
                            ...
                            //++
                            item_Cat.Reset();
                            item_Cat.SetRange(Code, "Purchase Line"."Item Category Code");
                            if item_Cat.Find('-') then begin
                                Cat_Des := item_Cat.Description;
                            end;
                            ...
                            NetUnitPrice := ("Purchase Line"."Direct Unit Cost" - (("Purchase Line"."Direct Unit Cost") * ("Purchase Line"."Line Discount %") / 100));

                            if ("Purchase Line".Type = "Purchase Line".Type::"Charge (Item)") then begin
                                "Purchase Line"."No." := "Purchase Line"."No.";
                                "Purchase Line".Description := "Purchase Line".Description;
                                SKU := '';
                                "Purchase Line".Quantity := 0;
                                "Purchase Line"."Unit of Measure" := '';
                                "Purchase Line"."Direct Unit Cost" := "Purchase Line"."Direct Unit Cost";
                                "Purchase Line"."Line Discount %" := 0;
                                NetUnitPrice := 0;
                                "Purchase Line"."Line Amount" := "Purchase Line"."Line Amount";
            ...
            begin
                ...
                PurchaseTable.SetRange(Code, "Purchaser Code");
                if PurchaseTable.FindSet() then begin
                    PurchaserName := PurchaseTable.Name;
                end;

                if ("Purchase Header"."No. of Archived Versions" > 0) then begin
                    Caption1 := 'Revised Purchase Order';
                end
                else begin
                    Caption1 := 'Purchase Order';
                end;
                ...
                if PurchCommentLine.FindFirst() then
                    repeat
                        ...
                        Comments[int] := PurchCommentLine.Comment;
    ...
    var
        ...
        ItemCatogory: Record "Item Category";
        Des: Text[100];
        PurchaseTable: Record "Salesperson/Purchaser";
        PurchaserName: Text[50];
        SKU: Code[20];
        NetUnitPrice: Decimal;
        Caption1: Text[100];
        VendorItemNo: Text[50];
        Comments: array[10] of text[250];
        int: Integer;
